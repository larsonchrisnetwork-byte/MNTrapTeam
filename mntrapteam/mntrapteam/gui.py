...
class MainWindow(QMainWindow):
    def __init__(self,db,rules,settings):
        ...
        self.setWindowTitle('MNTrapTeam 1.4'); self.resize(1320,820); self.setStyleSheet(DARK)
        self.tabs=QTabWidget(); self.setCentralWidget(self.tabs)
        self.dashboard=self.make_dashboard(); self.shooters=self.make_shooters(); self.imports=self.make_imports(); self.standings=self.make_standings(); self.projections=self.make_projections(); self.archive=self.make_archive(); self.diagnostics_tab=self.make_diagnostics(); self.settings_tab=self.make_settings()
    ...
    def make_diagnostics(self):
        w=QWidget(); v=QVBoxLayout(w); self.diag_label=QLabel(); self.diag_label.setTextFormat(Qt.RichText); v.addWidget(self.diag_label)
        self.issue_table=QTableView(); v.addWidget(QLabel('Unresolved import warnings')); v.addWidget(self.issue_table)
        b=QPushButton('Refresh diagnostics'); b.clicked.connect(self.refresh_diagnostics); v.addWidget(b); self.tabs.addTab(w,'Diagnostics'); return w
    def refresh_diagnostics(self):
        if not hasattr(self,'diag_label'): return
        d=self.db.diagnostics(); self.diag_label.setText('<h3>Database diagnostics</h3>'+''.join(f'<b>{k.replace("_"," ").title()}:</b> {v}<br>' for k,v in d.items()))
        issues=self.db.query('SELECT filename,issue_type,message,created_at FROM import_issues WHERE resolved=0 ORDER BY id DESC LIMIT 200')
        self.issue_table.setModel(DictModel(issues,[('filename','File'),('issue_type','Type'),('message','Message'),('created_at','Created')]))
    ...
    def refresh_all(self): self.refresh_dashboard(); self.refresh_shooters(); self.refresh_standings(); self.refresh_projection_shooters(); self.refresh_snapshots(); self.refresh_imports(); self.refresh_diagnostics()
