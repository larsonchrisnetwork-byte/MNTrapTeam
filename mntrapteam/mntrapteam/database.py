...
CREATE TABLE IF NOT EXISTS import_issues(id INTEGER PRIMARY KEY,import_id INTEGER,filename TEXT,row_number INTEGER,issue_type TEXT,message TEXT,resolved INTEGER DEFAULT 0,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(import_id) REFERENCES imports(id) ON DELETE CASCADE);
CREATE INDEX IF NOT EXISTS idx_stats_season ON season_stats(season); CREATE INDEX IF NOT EXISTS idx_import_issues_resolved ON import_issues(resolved); CREATE INDEX IF NOT EXISTS idx_scores_shooter ON scores(shooter_id); CREATE INDEX IF NOT EXISTS idx_scores_date ON scores(event_date);
"""
class Database:
    ...
    def record_import(self,filename,kind,sha256,rows_read,rows_imported,warnings):
        with self.connect() as con:
            import_id=con.execute('INSERT INTO imports(filename,kind,sha256,rows_read,rows_imported,warnings) VALUES(?,?,?,?,?,?)',(filename,kind,sha256,rows_read,rows_imported,'\n'.join(warnings))).lastrowid
            for message in warnings:
                con.execute('INSERT INTO import_issues(import_id,filename,issue_type,message) VALUES(?,?,?,?)',(import_id,filename,'warning',message))
            return import_id

    def diagnostics(self):
        return {
            'database':str(self.path),
            'shooters':self.scalar('SELECT COUNT(*) FROM shooters',default=0),
            'season_stats':self.scalar('SELECT COUNT(*) FROM season_stats',default=0),
            'scores':self.scalar('SELECT COUNT(*) FROM scores',default=0),
            'imports':self.scalar('SELECT COUNT(*) FROM imports',default=0),
            'open_import_issues':self.scalar('SELECT COUNT(*) FROM import_issues WHERE resolved=0',default=0),
        }
