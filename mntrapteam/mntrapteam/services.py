...
from .calculations import team_rankings, hoa, project, eligibility_completion
...
class TeamService:
    ...
    def season_rows(self,season):
        ...
        for r in rows:
            r['hoa']=hoa(r); r['eligibility']=self.rules.check(r)
            r['eligibility_completion']=eligibility_completion(r['eligibility'].progress)
            r['missing_requirements']=r['eligibility'].missing_count
        return rows
    def rankings(self,season,team):
        rows=team_rankings(self.season_rows(season),self.rules,team)
        selected=[r for r in rows if r['selected']]
        cutline=selected[-1]['hoa'] if selected else None
        for r in rows:
            r['cutline_hoa']=cutline
            r['hoa_gap_to_cut']=max(0.0,(cutline-r['hoa'])) if cutline is not None else 0.0
        return rows

    def shooter_detail(self,shooter_id,season):
        rows=self.db.query("SELECT s.*,st.* FROM shooters s LEFT JOIN season_stats st ON st.shooter_id=s.id AND st.season=? WHERE s.id=?",(season,shooter_id))
        if not rows:return None
        row=rows[0]; row['hoa']=hoa(row); row['eligibility']=self.rules.check(row); row['eligibility_completion']=eligibility_completion(row['eligibility'].progress)
        return row
